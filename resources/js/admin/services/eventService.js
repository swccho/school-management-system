...
export async function getList(params = {}) {
  ...
  const { data } = await api.get('/admin/events', { params: clean });
  return data;
}
...
function buildFormData(payload) {
  ...
  if (payload.location != null && payload.location !== '') form.append('location', payload.location);
  if (payload.start_datetime != null) form.append('start_datetime', payload.start_datetime);
  if (payload.end_datetime != null) form.append('end_datetime', payload.end_datetime);
...
export async function update(id, payload) {
  ...
  if (hasFile) {
    ...
    const { data } = await api.post(`/admin/events/${id}`, form, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return data;
  }
...
export async function unpublish(id) {
  const { data } = await api.post(`/admin/events/${id}/unpublish`);
  return data;
}
