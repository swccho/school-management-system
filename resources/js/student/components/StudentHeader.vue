      ...
      <div class="relative">
        ...
        <div v-if="notificationDropdownOpen" class="absolute right-0 top-full z-50 mt-1 w-80 rounded-xl border border-zinc-200 bg-white shadow-lg dark:border-zinc-800 dark:bg-zinc-900" @click.stop>
          <div class="max-h-96 overflow-y-auto p-2">
            <div v-if="notificationDropdownLoading" class="py-4 text-center text-sm text-zinc-500 dark:text-zinc-400">Loading…</div>
            <template v-else>
              <div v-if="!recentNotifications.length" class="py-4 text-center text-sm text-zinc-500 dark:text-zinc-400">No notifications</div>
              <template v-else>
                <div
                ...
                >
                  ...
                  <p class="mt-0.5 text-xs text-zinc-500 dark:text-zinc-400">{{ formatNotificationDate(n.created_at) }}</p>
          ...
          <div class="border-t border-zinc-200 p-2 dark:border-zinc-800">
            <router-link to="/student/notifications" class="block rounded-lg py-2 text-center text-sm font-medium text-zinc-600 dark:text-zinc-400" @click="notificationDropdownOpen = false">See all</router-link>
          </div>
        </div>
      </div>
      <span class="text-sm text-zinc-500 dark:text-zinc-400">{{ studentName }}</span>
      <button
        type="button"
...
const studentName = computed(() => authStore.studentName);
